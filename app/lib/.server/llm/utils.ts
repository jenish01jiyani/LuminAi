...
export function simplifyluminaiActions(input: string): string {
  // Using regex to match luminaiAction tags that have type="file"
  const regex = /(<luminaiAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/luminaiAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<luminaiAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</luminaiAction>`;
    });

  return `<luminaiArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</luminaiArtifact>`;
}

...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyluminaiActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyluminaiActions(content);

      content = content.replace(/<div class=\\"__luminaiThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`luminai failed to select files`);
  }

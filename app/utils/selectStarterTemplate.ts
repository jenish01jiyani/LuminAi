...
export async function getTemplates(templateName: string, title?: string) {
  ...
  // exclude    .luminai
  filteredFiles = filteredFiles.filter((x) => x.path.startsWith('.luminai') == false);

  // check for ignore file in .luminai folder
  const templateIgnoreFile = files.find((x) => x.path.startsWith('.luminai') && x.name == 'ignore');
...
<luminaiArtifact id="imported-files" title="${title || 'Importing Starter Files'}" type="bundled">
${filesToImport.files
  .map(
    (file) =>
      `<luminaiAction type="file" filePath="${file.path}">
${file.content}
</luminaiAction>`,
  )
  .join('\n')}
...
`;
  let userMessage = ``;
  const templatePromptFile = files.filter((x) => x.path.startsWith('.luminai')).find((x) => x.name == 'prompt');
